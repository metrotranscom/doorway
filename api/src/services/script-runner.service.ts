...
export class ScriptRunnerService {
  ...
  async transferJurisdictionData(
    ...
    // TODO: probably remove this depending on answer from product
    // // get reserved community types
    // const rcdQuery = `SELECT name, description, id FROM reserved_community_types WHERE jurisdiction_id = '${jurisdiction[0].id}'`;
    // const reservedCommunityTypes: {
    //   description: any;
    //   name: string;
    //   id: string;
    // }[] = await client.$queryRawUnsafe(rcdQuery);

    // // save reserved community types
    // if (reservedCommunityTypes.length) {
    //   console.log(
    //     `migrating ${reservedCommunityTypes.length} reserved community types`,
    //   );
    //   await this.prisma.reservedCommunityTypes.createMany({
    //     data: reservedCommunityTypes.map((rcd) => {
    //       return { ...rcd, jurisdictionId: doorwayJurisdiction.id };
    //     }),
    //   });
    // }
      ...
      listings.forEach(async (listing) => {
        ...
        const createdListing = await this.prisma.listings.create({
          data: {
            ...
            // TODO: add requested changes user ID when users have been carried over
        ...
        units.forEach(async (unit) => {
          ...
          if (unit['unit_rent_types']) {
            const fullRentType = rentTypes.find(
              (type) => type.id === unit['unit_rent_types'],
            );
