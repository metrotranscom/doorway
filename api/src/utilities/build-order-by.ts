...
/* 
  Constructs the "orderBy" part of the prisma query and maps the values to
  the appropriate multiselectQuestion field
*/
export const buildOrderByForMultiselectQuestions = (
  orderBy?: string[],
  orderDir?: OrderByEnum[],
  ...
  return orderBy.map((param, index) => {
    switch (param) {
      case MultiselectQuestionOrderByKeys.jurisdiction:
        return { jurisdiction: { name: orderDir[index] } };
      case MultiselectQuestionOrderByKeys.status:
        return { status: orderDir[index] };
      case MultiselectQuestionOrderByKeys.updatedAt:
        return { updatedAt: orderDir[index] };
      case ListingOrderByKeys.name:
      case undefined:
        return { name: orderDir[index] };
    }
  }) as Prisma.MultiselectQuestionsOrderByWithRelationInput[];
};
