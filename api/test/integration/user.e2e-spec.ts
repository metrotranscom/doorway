...
describe('User Controller Tests', () => {
  ...
  describe('warnUserOfDeletionCronJob endpoint', () => {
    ...
    it('should send warning email to only public users over the date', async () => {
      ...
      expect(mockSeSClient).not.toHaveReceivedCommandWith(SendEmailCommand, {
        FromEmailAddress: 'Doorway <no-reply@housingbayarea.org>',
        Destination: { ToAddresses: [userB.email] },
        Content: expect.anything(),
      });
      expect(mockSeSClient).not.toHaveReceivedCommandWith(SendEmailCommand, {
        FromEmailAddress: 'Doorway <no-reply@housingbayarea.org>',
        Destination: { ToAddresses: [userC.email] },
        Content: expect.anything(),
      });
      expect(mockSeSClient).not.toHaveReceivedCommandWith(SendEmailCommand, {
        FromEmailAddress: 'Doorway <no-reply@housingbayarea.org>',
        Destination: { ToAddresses: [userD.email] },
        Content: expect.anything(),
      });
      expect(mockSeSClient).toHaveReceivedCommandWith(SendEmailCommand, {
        FromEmailAddress: 'Doorway <no-reply@housingbayarea.org>',
        ...
        Content: {
          Simple: {
            Subject: {
              Data: 'Doorway Scheduled Account Removal Due to Inactivity',
            },
            Body: {
              Html: {
                Data: expect.anything(),
      ...
      expect(mockSeSClient).toHaveReceivedCommandWith(SendEmailCommand, {
        FromEmailAddress: 'Doorway <no-reply@housingbayarea.org>',
        Destination: { ToAddresses: [userE.email] },
        Content: {
          Simple: {
            Subject: {
              Data: 'Eliminación programada de cuenta de Doorway debido a inactividad',
            },
            Body: {
              Html: {
                Data: expect.anything(),
