  ...
  expectFailValues: Array<string> = []
    ...
    if(Array.isArray(propList)) {
      propList.forEach( (prop) => {
        (expectFailValues.includes(prop) ? expect(objValue[prop]).not : expect(objValue[prop])).toBe(rawValue[prop])
      })
    } else {
      Object.entries(propList).forEach( ([rawPropName, objPropName]) => {
        (expectFailValues.includes(objPropName) ? expect(objValue[objPropName]).not : expect(objValue[objPropName])).toBe(rawValue[rawPropName])
      })
    }
}
...
describe("CombinedListingTransformer", () => {
  it('should fully convert a raw object to Listing', () => {
      ...
      images: [
        {
          ordinal: 0,
          image: {
            fileId: "housingbayarea/slide_pacificoaks3_w43tf2","label":"cloudinaryBuilding",
    ...
    testObjectTransformation(raw.reserved_community_type, listing.reservedCommunityType, ReservedCommunityType, [
      'id',
      'name'
    ])
    ...
    listing.units.forEach( (unit, idx) => {
      ...
      // unit type
      testObjectTransformation(raw.units[idx].unitType, unit.unitType, UnitType, [
        'id',
        'name'
      ])
    })
